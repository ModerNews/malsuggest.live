...
from .models import AnimeObject, Node, Season, PagedResult, RankingType, Sorting, Genre
...
class Anime():
    ...
    def get_anime_genres(self, id: int) -> list[Genre]:
        id = str(id)
        uri = f'anime/{id}'
        params = {'fields': 'genres' }
        data = self._api_handler.call(uri, params=params)
        return [Genre(**genre) for genre in data['genres']] if 'genres' in data else None
